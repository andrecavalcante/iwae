...
class IWAE(nn.Module):
    ...
    def forward(self, x, num_samples):
        ...
        # handling under-flow 
        log_w = log_w - torch.max(log_w, dim=-1)[0].unsqueeze(1)
        
        # computing normalized weights
        normalized_w =  torch.exp(log_w)/ torch.sum(torch.exp(log_w), dim=-1).unsqueeze(1) # unsqueeze sum for broadcast
        ...
        w = torch.exp(log_w)
        log_px = torch.mean(torch.log((1/num_samples)*torch.sum(w, dim=-1))) # mean over batches
...
def main():
    ...
    train_dataset = torchvision.datasets.MNIST(root='/home/andre/Dropbox/iwae',train=True, transform=transforms.ToTensor(), download=True)
    ...
    for epoch in range(num_epochs):
        ...
        print('Epoch [{}/{}],  Log-likelihood: {:.3f}'.format(epoch + 1, num_epochs, log_px.item()))
